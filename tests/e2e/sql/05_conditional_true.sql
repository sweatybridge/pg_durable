...
CREATE TABLE test_cond_log (id SERIAL, branch TEXT);
...
INSERT INTO _test_state SELECT durable.start(
    durable.if(
        'SELECT true',
        'INSERT INTO test_cond_log (branch) VALUES (''then'')',
        'INSERT INTO test_cond_log (branch) VALUES (''else'')'
    ),
    'test-cond-true'
);
...
BEGIN
    ...
    SELECT branch INTO branch_val FROM test_cond_log ORDER BY id DESC LIMIT 1;
    IF branch_val != 'then' THEN
        RAISE EXCEPTION 'TEST FAILED: expected then branch, got %', branch_val;
    END IF;
    
    RAISE NOTICE 'TEST PASSED: conditional_true';
END $$;
